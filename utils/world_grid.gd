...
var highlighted_tile_global_pos: Vector2i
...
func get_tile(global_pos: Vector2i) -> TileInfo:
	var chunk_coords = _get_chunk_coords(global_pos)
	var local_pos = _get_local_pos(global_pos)
	var chunk = get_chunk(chunk_coords)
	if chunk:
		return chunk.get_tile_info(local_pos)
	return null

func highlight_tile(global_pos: Vector2i) -> void:
	var chunk_coords = _get_chunk_coords(global_pos)
	var local_pos = _get_local_pos(global_pos)
	var chunk = get_chunk(chunk_coords)

	var old_chunk_coords = _get_chunk_coords(highlighted_tile_global_pos)
	var old_local_pos = _get_local_pos(highlighted_tile_global_pos)
	var old_chunk = get_chunk(old_chunk_coords)
	if chunk && highlighted_tile_global_pos != global_pos:
